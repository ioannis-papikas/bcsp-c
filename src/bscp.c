...
int K;			// Number of sentences
int M;			// Number of propositions per sentence
...
int count(int *vector){
	...
	for(i=0;i<K;i++)
	{
		satisfied=0;
		for(j=0;j<M;j++)
			if ((Problem[i*M+j]>0 && vector[Problem[i*M+j]-1]==1) 
				||
				(Problem[i*M+j]<0 && vector[-Problem[i*M+j]-1]==-1))
				satisfied=1;
...
int valid(int *vector){
	...
	for(i=0;i<K;i++)
	{
		int satisfiable=0;
		for(j=0;j<M;j++)
			if ((Problem[i*M+j]>0 && vector[Problem[i*M+j]-1]>=0) 
				||
				(Problem[i*M+j]<0 && vector[-Problem[i*M+j]-1]<=0))
				satisfiable=1;
		if (satisfiable==0)
			return 0;
	}
